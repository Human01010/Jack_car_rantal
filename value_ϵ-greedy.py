...
DISCOUNT = 0.9  # 折扣因子
EPSILON = 0.1  # epsilon-greedy的epsilon值
EPSILON_MIN = 1e-4  # 收敛阈值
...
# epsilon-greedy 值迭代算法
def epsilon_greedy_value_iteration():
    global V
    iteration = 0
    while True:
        ...
        for state in states:
            ...
            # epsilon-greedy 策略
            if random.random() < EPSILON:  # 探索：随机选择动作
            ...
            # 更新价值函数
            delta = max(delta, abs(V[loc1_cars, loc2_cars] - best_action_value))
            V_new[loc1_cars, loc2_cars] = best_action_value
        V[:] = V_new  # 更新价值函数
        iteration += 1
        if delta < EPSILON_MIN or iteration >= NUM_ITER:  # 判断是否收敛
            break
    return V
...
# 执行 epsilon-greedy 值迭代
V_final = epsilon_greedy_value_iteration()
...
print(f"epsilon-greedy 值迭代算法执行时间: {execution_time:.4f} 秒")
